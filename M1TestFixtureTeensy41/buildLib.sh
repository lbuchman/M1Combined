...
if [ "$teensyType" = "teensy3" ]; then
    ...
    if [ "$teensyVersion" = 6 ]; then
        MCU=MK66FX1M0
        sed -i '/^OPTIONS =/ s/$/ -DSERIAL1_RX_BUFFER_SIZE=2048 -DSERIAL1_TX_BUFFER_SIZE=2048 -DSERIAL7_RX_BUFFER_SIZE=2048 -DSERIAL7_TX_BUFFER_SIZE=2048 -fsingle-precision-constant -mfloat-abi=hard -mfpu=fpv4-sp-d16/' ${COREPATH}/Makefile
...
if [ "$teensyType" = "teensy4" ]; then
    ...
    cd $COREPATH
    sed -i '/^CPPFLAGS =/ s/$/ -DSERIAL1_TX_BUFFER_SIZE=16384 -DSERIAL1_RX_BUFFER_SIZE=256 -DSERIAL2_TX_BUFFER_SIZE=256 -DSERIAL2_RX_BUFFER_SIZE=256 -DSERIAL7_TX_BUFFER_SIZE=256 -DSERIAL7_RX_BUFFER_SIZE=256/' ${COREPATH}/Makefile
    ...
    mv main.a ${ARDUINO_INST}/hardware/tools/arm/lib/gcc/arm-none-eabi/11.3.1/thumb/v7e-m+dp/hard/libteensy4${2}.a 
    set +e
    cp ${COREPATH}/pins_teensy.c.orig  ${COREPATH}/pins_teensy.c 2>/dev/null
    set -e
    exit 0
fi
