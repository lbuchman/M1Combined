...
int stm32mp1_dump_reg(const struct ddr_info *priv, const char *name) {
    ...
    if(name) {
        if(strcmp(name, base_name[DDR_BASE]) == 0) {
            ...
            if(strcmp(name, base_name[DDRPHY_BASE]) == 0) {
                filter = DDRPHY_BASE;
            }
    }
...
static void wait_operating_mode(struct ddr_info *priv, int mode) {
    ...
    if(mode == DDRCTRL_STAT_OPERATING_MODE_SR) {
        ...
        if(mode == DDRCTRL_STAT_OPERATING_MODE_NORMAL) {
...
void stm32mp1_ddr_init(struct ddr_info *priv, const struct stm32mp1_ddr_config *config) {
    ...
    if(config->c_reg.mstr & DDRCTRL_MSTR_DDR3) {
        ...
        if(config->c_reg.mstr & DDRCTRL_MSTR_LPDDR2) {
            if(bus_width == 32) {
            ...
            if(config->c_reg.mstr & DDRCTRL_MSTR_LPDDR3) {
                if(bus_width == 32) {
