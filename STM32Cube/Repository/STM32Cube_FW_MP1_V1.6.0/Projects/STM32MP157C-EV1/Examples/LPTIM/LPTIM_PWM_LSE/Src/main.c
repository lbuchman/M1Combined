...
int main(void)
{
  ...
  HAL_NVIC_SetPriority(USER_BUTTON_EXTI_IRQn, BSP_BUTTON_USER_IT_WAKEUP_PRIORITY, 0);
   ...
   */
  if (HAL_LPTIM_PWM_Start(&hlptim5, PeriodValue, PulseValue) != HAL_OK)
  {
    Error_Handler();
  }
  ...
  __set_BASEPRI((BSP_BUTTON_USER_IT_WAKEUP_PRIORITY + 1) << (8 - __NVIC_PRIO_BITS));
  ...
  if (HAL_LPTIM_PWM_Stop(&hlptim5) != HAL_OK)
  {
    Error_Handler();
  }
