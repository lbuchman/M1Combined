...
if (HAL_ADC_Start(&hadc2) != HAL_OK)
  ...
  while (1)
  {
    if (HAL_ADC_PollForConversion(&hadc2, 80) != HAL_OK)
    {
      Error_Handler();
    }

    /* Read the converted value */
    uwConvertedValue = HAL_ADC_GetValue(&hadc2);

    /* Convert the result from 16 bit value to the voltage dimension (mV unit) */
    /* Vref = 2.9 V */
    uwInputVoltage = uwConvertedValue * VDDA_APPLI;
    uwInputVoltage = uwInputVoltage / 0xFFF0;

    /* check value with +- 100mV tolerance */
    if ((uwInputVoltage > (VDDA_APPLI/2 - 100)) && (uwInputVoltage < (VDDA_APPLI/2 + 100)))
    {
      BSP_LED_On(LED4); /* OK */
    }
    else
