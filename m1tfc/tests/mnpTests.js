...
module.exports = class MnpTests {
    constructor(db, log) {
        ...
        this.mnpIo = [
            ...
            { name: 'rd1rled', pinType: 'INPUT', cmdRead: 'rd1rled', pinN: 5, mnpPinName: 'RD1_RLED', inverted: 1 },
            { name: 'rd1gled', pinType: 'INPUT', cmdRead: 'rd1gled', pinN: 11, mnpPinName: 'RD1_GLED', inverted: 1 },
            { name: 'rd1bz', pinType: 'INPUT', cmdRead: 'rd1bz', pinN: 40, mnpPinName: 'WGD1_BPR', inverted: 1 },
            // Reader 2
            ...
            { name: 'Rd2Rled', pinType: 'INPUT', cmdRead: 'rd2rled', pinN: 12, mnpPinName: 'RD2_RLED', inverted: 1 },
            { name: 'Rd2Gled', pinType: 'INPUT', cmdRead: 'rd2gled', pinN: 30, mnpPinName: 'RD2_GLED', inverted: 1 },
            { name: 'Rd2bz', pinType: 'INPUT', cmdRead: 'rd2bz', pinN: 39, mnpPinName: 'WGD2_BPR', inverted: 1 },
            // Relay 1
    ...
    async testLogicalState(thisIo, thisMnpIo, value, inverted) {
    ...
    async run() {
        ...
        for (const thisIo of this.mnpIo) {
            ...
            if (thisIo.pinType === 'OUTPUT') {
                // this.logger.info(`Testing ${thisMnpIo.name}`);
                // eslint-disable-next-line no-await-in-loop
                if (!await this.testLogicalState(thisIo, thisMnpIo, 1, thisIo.inverted)) {
                    ret = false;
                    testPassed = false;
                }
                // eslint-disable-next-line no-await-in-loop
                if (!await this.testLogicalState(thisIo, thisMnpIo, 0, thisIo.inverted)) {
                    ret = false;
                    testPassed = false;
                }
